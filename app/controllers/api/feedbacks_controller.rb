...
class Api::FeedbacksController < ApplicationController
  ...
  # New method to store match feedback
  def store_match_feedback
    match_feedback_params = params.require(:match_feedback).permit(:user_id, :match_id, :feedback)

    # Verify that the match_id exists in the "matches" table.
    match = Match.find_by(id: match_feedback_params[:match_id])
    unless match
      render json: { error: 'Match not found.' }, status: :not_found
      return
    end
    ...
    # Verify that the user_id exists in the "users" table.
    user = User.find_by(id: match_feedback_params[:user_id])
    unless user
      render json: { error: 'User not found.' }, status: :not_found
      return
    end
    ...
    # Create a new entry in the match_feedback table
    match_feedback = MatchFeedback.new(match_feedback_params)

    if match_feedback.save
      # Placeholder for logic to adjust the matching algorithm
      # ...

      render json: { status: 201, message: 'Match feedback submitted successfully.' }, status: :created
    else
      render json: { error: match_feedback.errors.full_messages }, status: :unprocessable_entity
    end
  rescue Pundit::NotAuthorizedError
    render json: { error: 'You are not authorized to perform this action' }, status: :forbidden
  rescue => e
  ...
  # New private method to permit match_feedback parameters
  def match_feedback_params
    params.require(:match_feedback).permit(:user_id, :match_id, :feedback)
  end
end
