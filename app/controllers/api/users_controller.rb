class Api::UsersController < Api::BaseController
  before_action :doorkeeper_authorize!, only: %i[index show update_profile matches update_preferences complete_profile swipes get_potential_matches]
  before_action :set_user, only: [:matches, :update_preferences, :complete_profile, :get_potential_matches]
  before_action :authenticate_user!, only: [:update_preferences, :get_potential_matches] # Added get_potential_matches to the list
  ...
  require_dependency 'user_service/index' # Import the UserService::Index class
  ...
  def get_potential_matches
    ...
    begin
      # Assuming MatchmakingService exists and has a method to get potential matches
      potential_matches = MatchmakingService.new.get_potential_matches_for(@user)
